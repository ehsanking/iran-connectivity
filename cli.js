...
class IranCheckCLI {
    ...
    printHelpfulTools(targetIp = '') {
        ...
        const tools = [
            ...
            { name: 'OONI CLI', url: 'https://ooni.org/install/cli/', reason: `One-line install+run: curl -fsSL https://get.ooni.org | sh && ooniprobe run web_connectivity --input ${safeIp}` },
...
program
    .command('ooni <targetIp>')
    .description('Print one-line OONI CLI install and run command for a target IP/domain')
    .action((targetIp) => {
        const cli = new IranCheckCLI();
        cli.printBanner();
        const safeTarget = cli.validateIp(targetIp) ? targetIp : targetIp.trim();
        if (!safeTarget) {
